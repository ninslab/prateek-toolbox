...
for folders in $srcDir/$fPrefix* ; do
	...
	# Binarize Image by Label Value to Obtain ROI Extraction Mask
	echo fslmaths $extractedROIFolder/$labelName.nii.gz -bin $extractedROIFolder/${labelName}_Mask.nii.gz >> $srcDir/LogFile.txt
	fslmaths $extractedROIFolder/$labelName.nii.gz -bin $extractedROIFolder/${labelName}_Mask.nii.gz
	
	echo
	echo "Completed Mask Generation from Segmentation File."
	echo
	
	# Extract Transform File Name
	inputFileName=$extractedROIFolder/${labelName}_Mask.nii.gz
	outputFileName=$extractedROIFolder/${labelName}_Mask_RegToQSM.nii.gz
	transformFile=$(ls $registeredImageFolder/*0GenericAffine.mat)
	referenceFile=$(ls $registeredImageFolder/*_Warped.nii.gz)
	
	# Transform Image Mask to Obtain ROI Extraction Mask
	...
	echo
	echo "Completed Mask Transformation to QSM Image Co-ordinates."
	echo
	
	# Apply Generated ROI Mask to QSM Image
	echo mri_mask $inputImageFolder/${folderName}_QSM.nii $extractedROIFolder/${labelName}_Mask_RegToQSM.nii.gz $extractedROIFolder/${folderName}_QSM_${labelName}.nii.gz >> $srcDir/LogFile.txt
	mri_mask $inputImageFolder/${folderName}_QSM.nii $extractedROIFolder/${labelName}_Mask_RegToQSM.nii.gz $extractedROIFolder/${folderName}_QSM_${labelName}.nii.gz

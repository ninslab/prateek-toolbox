function performallfmrimrsproc(tr_value_num, slicenumbervalue_num, workingdir, getlabelval)
...
MRS_filenames = mrs_filenames(string,directory);
filenames = MRS_filenames.(char(string));
MRS_mask_raw(subject_dir,filenames(:,1),filenames(:,2),string);
mrs_mask_raw=(repmat((strcat(string,'_T1mask_raw.nii')),length(subject_dir),1));
MRS_mask_normalize(subject_dir,mrs_mask_raw,filenames(:,2),string);
results_file_folder_fmri_mrs(subject_dir,subject_code,string);

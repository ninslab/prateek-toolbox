function filestructgenerate24(dir_value)

h.figure = figure('position' , [600 440 400 200]);
set(h.figure, 'Color', [0.43 0.76 0.70]);

h.subjectcodeprompt = uicontrol(h.figure ,'Style','text',...
    'String','Subject Code',...
    'Position',[20 120 150 20]);

h.subjectcodeaddition = uicontrol(h.figure ,'Style','edit',...
    'String','',...
    'Position',[200 120 150 20]);

h.numberofsubjects = uicontrol(h.figure ,'Style','text',...
    'String','Upload Data',...
    'Position',[20 80 150 20]);

h.fmridata = uicontrol(h.figure ,'Style','pushbutton',...
    'String','fMRI',...
    'Position',[200 80 60 20]);

set(h.fmridata,'callback', {@fmridataproc,h});

h.mri_fmri_data = uicontrol(h.figure ,'Style','pushbutton',...
    'String','MRI (fMRI)',...
    'Position',[270 80 80 20]);

set(h.mri_fmri_data,'callback', {@mri_fmri_dataproc,h});

h.exceldata_fmri = uicontrol(h.figure ,'Style','pushbutton',...
    'String','Onset details (.xlsx)',...
    'Position',[200 55 150 20]);

set(h.exceldata_fmri, 'callback', {@excel_response_call,h})

h.mrs_data = uicontrol(h.figure ,'Style','pushbutton',...
    'String','MEG',...
    'Position',[200 30 60 20]);

set(h.mrs_data,'callback', {@meg_dataproc,h});

h.mri_meg_data = uicontrol(h.figure ,'Style','pushbutton',...
    'String','MRI (MEG)',...
    'Position',[270 30 80 20]);

h.addsubjectcode = uicontrol(h.figure ,'Style','pushbutton',...
    'String','Add',...
    'Position',[200 5 150 20]);

set(h.mri_meg_data,'callback', {@meg_mri_dataproc,h});

% set(h.mri_meg_data,'callback', {@mri_mrs_dataproc,h});
set(h.addsubjectcode,'callback', {@addsubjectcodeproc_meg_fmri,h});
% set(h.addsubjectcode ,'callback', {@addsubjectcodeprocmeg_fmri ,h});

end